...
public class DefaultBeanFactory implements BeanFactory {
    ...
    public DefaultBeanFactory(Set<Class<?>> preInstantiatedBeans) {
        ...
        Map<Class<?>, Method> methodBeans = preInstantiatedBeans.stream()
                ...
                .collect(Collectors.toMap(Method::getReturnType, x-> x));
                ...
                .map(method -> {
                    try {
                        Object[] params = Arrays.stream(method.getParameterTypes())
                                .map(x-> {
                                    if(getBean(x) == null){
                                        Method method1 = methodBeans.get(x);
                                        Object ob = method1.invoke(getOrCreateBean(x));
                                        beans.put(ob.getClass(), ob);
                                        return ob;
                                    }
                                    return getBean(x);
                                })
                        ...
                        return method.invoke(getOrCreateBean(method.getClass()), params);
...
//
//
//                            return method.invoke(getOrCreateBean(x), params);
//                        } catch (IllegalAccessException | InvocationTargetException e) {
//                            e.printStackTrace();
//                            throw new RuntimeException();
//                        }
